	...
	for(var/datum/sound_template/run_template in run_queue)
		if(!run_hearers) 										// Initialize for handling next template
			...
			if(run_template.range) 								// ranging
				var/datum/shape/rectangle/zone = RECT_OLD(run_template.x, run_template.y, run_template.range * 2, run_template.range * 2)
				run_hearers |= SSquadtree_old.players_in_range(zone, run_template.z)
			if(MC_TICK_CHECK)
				return
